...
/**
 * Tests that particle-to-particle potentials are calculated correctly at the
 * leaf layer.
 */
template <int p>
void testSolver( int points_per_proc_in, bool balanced )
{
    ...
    if ( red_factor < 2 )
        red_factor = 2;
    auto tree = Canopy::createSolver<TEST_MEMSPACE, TEST_EXECSPACE, MD_f,
                                     cells_per_tile, p>(
        global_low_corner, global_high_corner, leaf_tiles, red_factor,
        MPI_COMM_WORLD );

    // The tree depth should always be at least three, but this check is here
    // just in case. If the depth is less than 3, this test may not work
    // correctly. if (rank == 0) printf("R%d: num tree layers: %d\n", rank,
    // tree->numLayers()); ASSERT_EQ(tree->numLayers(), 3) <<
    // "testMultipole2Local: Error: Solver depth must be depth 3.";

    // Create the data on rank 0. It will automatically be distributed correctly
    // when filled into the tree. There must be enough particles so that the
    // target point resides in a cell that has been activated in the mesh. This
    // won't be a problem in the "real" code because we only evaluate locals
    ...
    Kokkos::Array<scalar_type, 6> coord_bounds = {
        -bound_val, -bound_val, -bound_val, bound_val, bound_val, bound_val };
    ...
    // Kokkos::Array<scalar_type, 6> coord_bounds1 = {2.3, 2.3, 2.3, bound_val,
    // bound_val, bound_val};
    fillRandomCoordinates( cart_coords, coord_bounds, 123 );
    fillRandomScalar( q, charge_bounds, 321 );

    particle_aosoa_type_f_h particle_aosoa_host( "particle_aosoa",
                                                 owned_points );
    ...
    for ( int i = 0; i < owned_points; ++i )
    {
        ...
        id_slice_host( i ) = i;
        // printf("R%d: initial particle: p(%0.3lf, %0.3lf, %0.3lf),
        // q(%0.3lf)\n", rank,
    ...
    Kokkos::View<scalar_type*, Kokkos::HostSpace> direct_potentials(
        "direct_potentials", owned_points );
    Kokkos::View<scalar_type* [3], Kokkos::HostSpace> direct_forces(
        "direct_forces", owned_points );
    ...
    for ( int this_pid = 0; this_pid < owned_points; this_pid++ )
    {
        ...
        //         Kokkos::floor((pos_slice_host(this_pid, dim) -
        //         global_low_corner[dim]) / cell_size[dim]) );
        // }
        // printf("this_pid(%d): (%d, %d, %d)\n", this_pid, this_cell_ijk[0],
        // this_cell_ijk[1], this_cell_ijk[2]);

        // Iterate over all particles inserted into the mesh. If it falls into a
        // cell within 2 cells of the target point's cell, skip it. If not, add
        // its contribution to the potential at the target point.
        for ( int other_pid = 0; other_pid < owned_points; other_pid++ )
        {
            ...
            //         Kokkos::floor((pos_slice_host(other_pid, dim) -
            //         global_low_corner[dim]) / cell_size[dim]) );
            // }
            ...
            scalar_type dx =
                pos_slice_host( other_pid, 0 ) - pos_slice_host( this_pid, 0 );
            scalar_type dy =
                pos_slice_host( other_pid, 1 ) - pos_slice_host( this_pid, 1 );
            scalar_type dz =
                pos_slice_host( other_pid, 2 ) - pos_slice_host( this_pid, 2 );
            ...
            scalar_type fp =
                -1 * q_h( this_pid ) * q_h( other_pid ) * dist_inv3;
    ...
    auto aosoa_device = std::make_shared<particle_aosoa_type_f>(
        "aosoa_device", particle_aosoa_host.size() );
    ...
    // Check mesh information for leaf layer. Must be done after solve or else
    // the tree has not yet been constructed.
    int cells_per_dimension_leaf = cells_per_tile * leaf_tiles;
    Kokkos::Array<scalar_type, 3> cell_size;
    for ( int i = 0; i < 3; ++i )
    {
        cell_size[i] = ( global_high_corner[i] - global_low_corner[i] ) /
                       cells_per_dimension_leaf;
    }
    ASSERT_EQ( tree->layer( 0 )->cellsPerDim(), cells_per_dimension_leaf )
        << "testMultipole2Local: Error: Unexpected cells_per_leaf_dimension";
    ASSERT_EQ( tree->layer( 0 )->tilesPerDim(), leaf_tiles )
        << "testMultipole2Local: Error: Unexpected leaf_tiles";
    ASSERT_EQ( tree->layer( 0 )->cellSize(), cell_size )
        << "testMultipole2Local: Error: Unexpected cell_size";
    ...
    for ( int i = 0; i < owned_points; i++ )
    {
        ...
        EXPECT_NEAR( solver_potential, direct_potential, 0.003 )
            << " at particle " << i;
        ...
        for ( int d = 0; d < 3; d++ )
        {
            ...
            EXPECT_NEAR( solver_force, direct_force, 0.6 )
                << " at particle " << i;
        ...
        // direct_potential, mesh_potential);
    }
    if ( owned_points > 0 )
...
TEST( SingleSolve, testSingleSolve_balanced ) { testSolver<6>( 500, true ); }
TEST( SingleSolve, testSingleSolve_unbalanced ) { testSolver<6>( 500, false ); }
