...
macro(Canopy_add_tests_nobackend)
  cmake_parse_arguments(Canopy_UNIT_TEST "" "PACKAGE" "NAMES;LABELS" ${ARGN})
  foreach(_test ${Canopy_UNIT_TEST_NAMES})
    ...
    if(Canopy_UNIT_TEST_LABELS)
      set_property(TEST ${_target} PROPERTY LABELS ${Canopy_UNIT_TEST_LABELS})
    endif()
    if(VALGRIND_FOUND)
      ...
      if(Canopy_UNIT_TEST_LABELS)
        set_property(TEST ${_target}_valgrind PROPERTY LABELS ${Canopy_UNIT_TEST_LABELS})
      endif()
    endif()
    ...
    foreach(_test ${CANOPY_UNIT_TEST_NAMES})
      ...
      if(CANOPY_UNIT_TEST_MPI)
        foreach(_np ${CANOPY_UNIT_TEST_MPIEXEC_NUMPROCS})
          ...
          if(CANOPY_UNIT_TEST_LABELS)
            set_property(TEST ${_target}_np_${_np} PROPERTY LABELS ${CANOPY_UNIT_TEST_LABELS})
          endif()
        endforeach()
      else()
        if(_device STREQUAL THREADS OR _device STREQUAL OPENMP)
          foreach(_thread ${CANOPY_UNIT_TEST_NUMTHREADS})
            ...
            if(CANOPY_UNIT_TEST_LABELS)
              set_property(TEST ${_target}_nt_${_thread} PROPERTY LABELS ${CANOPY_UNIT_TEST_LABELS})
            endif()
            if(VALGRIND_FOUND)
              ...
              if(CANOPY_UNIT_TEST_LABELS)
                set_property(TEST ${_target}_nt_${_thread}_valgrind PROPERTY LABELS ${CANOPY_UNIT_TEST_LABELS})
              endif()
            endif()
          endforeach()
        else()
