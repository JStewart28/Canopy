        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA(const int index) {
                if (cid_tid_map.valid_at(index))
                {
                    ...
                    // if (rank == 0) printf("R%d: getting at tid, cid: %d, %d\n", rank, ids.first, ids.second);
                    // cell_data.setTuple(tkey, tp);
                }
            }
        );
    ...
    void populateCells(const data_aosoa_type data_aosoa, AggregationFunctor functor)
    {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA( const int pid ) {
                ...
                if (!result.success())
                {
                    printf("Error adding key to map\n");
                }
            } );
...
class Octree
{
  public:
    ...
    //! Self type
    using octree_type = Octree<ExecutionSpace, MemorySpace, DataTypes, EntityType,
    ...
    void add_layer(const int tiles_per_dim, const int halo_width)
    {
        ...
        auto layer = makeTreeLayer<octree_type, cell_per_tile_dim>(
  ...
  private:
    ...
    // Tree layers.
    std::vector<std::shared_ptr<TreeLayer<octree_type, cell_per_tile_dim>>> _tree;
