                             ...
                             const std::string& label = "slice_mirror" )
{
    ...
    if constexpr ( rank == 1 )
    {
        ...
        Kokkos::View<value_type*, src_memory_space> src_view( label + "_src",
                                                              ext0 );

        Kokkos::parallel_for(
            "SliceToView_1D",
            Kokkos::RangePolicy<src_execution_space>( 0, ext0 ),
            KOKKOS_LAMBDA( int i ) { src_view( i ) = slice( i ); } );
        Kokkos::fence();

        typename MirrorViewType<SliceType, MemorySpace>::type dst_view( label,
                                                                        ext0 );
    ...
    else if constexpr ( rank == 2 )
    {
        ...
        typename MirrorViewType<SliceType, MemorySpace>::type dst_view(
            label, ext0, ext1 );
