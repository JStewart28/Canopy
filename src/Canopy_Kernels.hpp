...
Kokkos::complex<double> Ynm(int n, int m, double theta, double phi)
{
    ...
    // Remark before eq 3.27
    norm /= Kokkos::sqrt( ( ( 2.0 * n + 1 ) / ( 4.0 * pi ) ));

    double P = Pnm_impl(n, m, cos(theta));
    double real = cos(static_cast<double>(m) * phi);
    double imag = sin(static_cast<double>(m) * phi);
    return Kokkos::complex<double>(norm * P * real,
                                norm * P * imag);
}
