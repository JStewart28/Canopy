...
auto read_w(const std::string& filename)
{
    using ViewType_h = Kokkos::View<double**[2], Kokkos::HostSpace>;
    using ViewType_d = Kokkos::View<double**[2], MemorySpace>;

    // Call the function with the explicit template type
    auto read_view = Utils::readViewFromFile<ViewType_h>(filename, 2);
    int dim0 = read_view.extent(0);
    int dim1 = read_view.extent(1);
    ViewType_d w = ViewType_d("vorticities", dim0, dim1, 2);
...
template <class MemorySpace>
auto read_z(const std::string& filename)
{
    using ViewType_h = Kokkos::View<double**[3], Kokkos::HostSpace>;
    using ViewType_d = Kokkos::View<double**[3], MemorySpace>;

    // Call the function with the explicit template type
    auto read_view = Utils::readViewFromFile<ViewType_h>(filename, 3);
    int dim0 = read_view.extent(0);
    int dim1 = read_view.extent(1);
    ViewType_d z = ViewType_d("positions", dim0, dim1, 3);
    auto temp = Kokkos::create_mirror_view(read_view);
    Kokkos::deep_copy(temp, read_view);
